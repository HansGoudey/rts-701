...
func process_navigation(delta:float) -> void:
	if find_path:
		...
		path = nav.get_simple_path(initial_pos, target_location)
		find_path = false
	if path.size() > 0:
...
func get_z() -> float:
	...
	return isect_line_plane_v3(to, from, Vector3(0, 0, 0), Vector3(0, 1, 0)).z
...
func move_unit(from: Vector3, to: Vector3, delta:float):
	...
	translation += v # need to account for change in the z
	if translation.distance_squared_to(to) < 9: # this constant
		path.remove(0)
